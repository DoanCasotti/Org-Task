    ...
    <div className="bg-white border border-gray-200 rounded-lg p-3 hover:shadow-md transition-shadow duration-200 group">
      <div className="flex items-start justify-between gap-2 mb-2">
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button variant="ghost" size="sm" className="md:opacity-0 md:group-hover:opacity-100 transition-opacity h-6 w-6 p-0 shrink-0">
          ...
          <DropdownMenuContent align="end" className="w-40">
            ...
            <DropdownMenuSeparator />
            <DropdownMenuItem onClick={() => onStatusChange('todo')} className={task.status === 'todo' ? 'bg-gray-100' : ''}>A Fazer</DropdownMenuItem>
            <DropdownMenuItem onClick={() => onStatusChange('in_progress')} className={task.status === 'in_progress' ? 'bg-gray-100' : ''}>Em Progresso</DropdownMenuItem>
            <DropdownMenuItem onClick={() => onStatusChange('done')} className={task.status === 'done' ? 'bg-gray-100' : ''}>Concluído</DropdownMenuItem>
            <DropdownMenuSeparator />
      ...
      <div className="flex items-center justify-between gap-2 flex-wrap">
        ...
        <div className="flex items-center gap-2">
          ...
          {assignee && (
            <div className="flex items-center gap-1" title={assignee.username || ''}>
              {assignee.avatar_url ? (
