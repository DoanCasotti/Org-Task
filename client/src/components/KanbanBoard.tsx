...
interface KanbanBoardProps {
  tasks: Task[];
  onReorder: (tasks: { id: string; status: TaskStatus; order: number }[]) => void;
...
export function KanbanBoard({ tasks, onReorder, onEditTask, onDeleteTask, onStatusChange }: KanbanBoardProps) {
  ...
  const handleDragEnd = (result: DropResult) => {
    ...
    if (sourceStatus === destStatus) {
      ...
      const updates = sourceTasks.map((t, i) => ({ id: t.id, status: sourceStatus, order: i }));
      onReorder(updates);
    } else {
